    ...
    return (
      <div className="flex flex-col items-center justify-center h-full max-w-[650px] mx-auto gap-6 select-none animate-in fade-in duration-200">
        <Card className="w-full border border-neutral-200 dark:border-neutral-800 bg-white/60 dark:bg-neutral-900/30 shadow-xl rounded-3xl overflow-hidden p-7 flex flex-col items-center gap-6">
          ...
          <div className="flex flex-col items-center gap-1.5 text-center mt-1 max-w-full">
          ...
          <div className="w-full space-y-1.5">
            <div className="w-full bg-neutral-200 dark:bg-neutral-800 rounded-full h-2.5 overflow-hidden border border-neutral-200/60 dark:border-neutral-700/60">
              <div 
                className="bg-emerald-500 h-full rounded-full transition-all duration-300 ease-out" 
                style={{ width: `${progressPercent}%` }}
              />
            </div>
            <div className="flex justify-between text-xs font-semibold text-neutral-500 dark:text-neutral-400 px-1">
      ...
      <div className="w-[340px] flex-shrink-0 border border-neutral-200 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-950/40 p-4.5 rounded-2xl flex flex-col gap-4 overflow-y-auto">
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="pl-9.5 bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800 focus-visible:ring-emerald-500 text-sm h-9.5 rounded-xl"
          />
        </div>

        {tasks.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center py-10 px-4 border border-dashed border-neutral-200 dark:border-neutral-800 rounded-xl text-center gap-3">
            <CheckCircle2 className="h-9 w-9 text-emerald-500 animate-bounce" />
            <div className="space-y-1">
              <p className="text-sm font-bold text-neutral-900 dark:text-neutral-100">¡Sin tareas pendientes!</p>
              <p className="text-xs text-neutral-500 dark:text-neutral-400">Crea requerimientos en tus proyectos antes de iniciar un desafío.</p>
            </div>
          </div>
        ) : filteredTasks.length === 0 ? (
          <p className="text-center text-sm text-neutral-400 dark:text-neutral-500 py-10">
          ...
          <div className="flex flex-col gap-2.5 flex-1">
            {filteredTasks.map(t => (
              <Card
                key={t.id}
                className={`cursor-pointer border transition-all duration-200 shadow-none rounded-xl hover:bg-neutral-100/60 dark:hover:bg-neutral-900/40 ${
                  selectedTaskId === t.id
                ...
                <CardContent className="p-3.5 flex flex-col gap-1.5">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {t.status === 'in-progress' && (
                      <Badge variant="outline" className="border-amber-500/20 bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[10px] py-0.5 px-1.5 font-bold uppercase select-none rounded-md">
                        En curso
                      </Badge>
                    )}
                  </div>
                  <h4 className={`font-bold text-sm leading-snug break-words ${
                    selectedTaskId === t.id ? 'text-emerald-600 dark:text-emerald-400' : 'text-neutral-900 dark:text-neutral-100'
                  }`}>
                    {t.title}
                  </h4>
                </CardContent>
        ...
        {selectedTask ? (
          <div className="flex flex-col gap-5 h-full animate-in fade-in slide-in-from-right-1 duration-200">
            ...
            <div className="space-y-1.5">
              ...
              {selectedTask.description && (
                <p className="text-sm text-neutral-600 dark:text-neutral-300 pl-7 leading-relaxed">
                  {selectedTask.description}
                </p>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {presets.map(p => {
                  ...
                  return (
                    <Card
                      key={p.label}
                      className={`cursor-pointer border shadow-none text-center p-3.5 transition-all duration-200 rounded-xl hover:bg-neutral-100/60 dark:hover:bg-neutral-900/50 ${
                        isActive
                    ...
                    >
                      <CardContent className="p-0 flex flex-col items-center gap-1 justify-center">
                        <span className="text-base font-bold">{p.label}</span>
                        <span className="text-xs text-neutral-500 dark:text-neutral-400 font-semibold">{p.sub}</span>
                      </CardContent>
                    </Card>
                  );
                })}
              ...
              <div className="flex items-center gap-3">
                <div className="relative max-w-[130px]">
                  <Input
                    ...
                    className="bg-white dark:bg-neutral-950 border-neutral-200 dark:border-neutral-800 focus-visible:ring-emerald-500 text-sm font-bold h-10 pr-7 rounded-xl"
                  />
                  <span className="absolute right-3 top-2.5 text-xs text-neutral-400 font-bold select-none">
                ...
                <div className="flex items-center border border-neutral-200 dark:border-neutral-800 rounded-xl p-1 bg-neutral-100/60 dark:bg-neutral-900/60">
                  <Button
                    variant="ghost"
                    size="sm"
                    className={`h-8 px-3.5 text-xs rounded-lg font-bold ${focusType === 'minutes' ? 'bg-white dark:bg-neutral-950 text-neutral-900 dark:text-neutral-50 shadow-sm' : 'text-neutral-500 hover:text-neutral-900'}`}
                    onClick={() => {
                      setFocusType('minutes');
                      if (focusType === 'hours') {
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className={`h-8 px-3.5 text-xs rounded-lg font-bold ${focusType === 'hours' ? 'bg-white dark:bg-neutral-950 text-neutral-900 dark:text-neutral-50 shadow-sm' : 'text-neutral-500 hover:text-neutral-900'}`}
                    onClick={() => {
                      setFocusType('hours');
            ...
            <div className="mt-auto pt-4 flex items-center justify-end">
              <Button 
                size="lg" 
                className="h-11 text-sm font-bold gap-2 px-7 shadow-md rounded-xl"
                onClick={handleStart}
              >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 gap-4">
            <div className="p-4.5 bg-neutral-100 dark:bg-neutral-900 rounded-full text-neutral-400 dark:text-neutral-500">
              <Trophy className="h-12 w-12 text-amber-500/80" />
            </div>
            <div className="max-w-[420px] space-y-2">
              <h3 className="font-bold text-lg text-neutral-900 dark:text-neutral-100">Comienza tu Desafío</h3>
