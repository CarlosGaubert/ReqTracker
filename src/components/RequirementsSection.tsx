...
import { Plus, Trash2, Bell, BellOff, Calendar, AlertCircle } from 'lucide-react';
  ...
  const getUrgencyInfo = (req: Requirement) => {
    ...
    if (daysDiff < 0) {
      return {
        ...
        badge: (
          <Badge variant="outline" className="border-red-200 dark:border-red-950 bg-red-500/10 text-red-600 dark:text-red-400 gap-1 py-0.5 px-2 select-none shadow-none text-[10px] font-medium">
      ...
      return {
        ...
        badge: (
          <Badge variant="outline" className="border-amber-250 dark:border-amber-950 bg-amber-500/10 text-amber-600 dark:text-amber-400 gap-1 py-0.5 px-2 select-none shadow-none text-[10px] font-medium">
            <AlertCircle className="h-3 w-3" />
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      <div className="flex items-start justify-between">
        ...
        <Button size="sm" className="h-9 gap-1.5" onClick={() => setIsModalOpen(true)}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-8 gap-4">
          ...
          <div className="max-w-[320px] space-y-1.5">
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400">
              No hay requerimientos ni tareas pendientes para este proyecto. Crea uno nuevo para comenzar.
            </p>
          </div>
        </div>
      ) : (
                ...
                <div className="flex-1 flex flex-col gap-1.5 min-w-0">
                  ...
                  {req.description && (
                    <p className={`text-xs leading-relaxed break-words select-text ${
                      req.status === 'done' ? 'text-neutral-400 dark:text-neutral-500' : 'text-neutral-500 dark:text-neutral-400'
                    }`}>
                      {req.description}
                    </p>
                  )}
                  <div className="flex flex-wrap items-center gap-2 mt-1">
                    <Badge variant="outline" className="border-neutral-200 dark:border-neutral-800 text-neutral-500 dark:text-neutral-400 gap-1 py-0.5 px-2 select-none shadow-none text-[10px] font-medium bg-transparent">
